...
class MqttClient(object):
    def __init__(self, client_id, trust_chain, cert, key):
    ...
    def connect(self, host, port, keep_alive=120):
        logging.debug("Mqtt connecting to {0}:{1}".format(host, port))
...
def on_connect(client, userdata, flags, rc):
    if rc != 0:
        logging.error(f" Mqtt {client._client_id} on_connect failed: {rc} {mqtt.connack_string(rc)}")
    else:
        logging.info(f"Mqtt {client._client_id} on_connect success: {rc} {mqtt.connack_string(rc)}")


def on_message(client, userdata, message):
    # logging.debug(f'Mqtt {client._client_id} on_message {message.topic} {str(message.qos)} {str(message.payload)}')
    pass


def on_publish(client, userdata, mid):
    # logging.debug(f"Mqtt {client._client_id} on_publish {userdata} {mid}")
    pass


def on_subscribe(client, userdata, mid, granted_qos):
    # logging.debug("Mqtt on_subscribe " + str(mid) + " " + str(granted_qos))
    pass


def on_disconnect(client, userdata, rc):
    if rc != 0:
        logging.error(f"Mqtt {client._client_id} on_disconnect failed: {rc}")
    else:
        logging.info(f"Mqtt {client._client_id} on_disconnect success: {rc}")

def on_log(mqttc, obj, level, string):
    logging.debug(f"Mqtt on_log {string}")
