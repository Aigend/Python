...
import logging
from .commonlib import show_json
...
class MyRequest(r.Session):
    ...
    def requset_param_logs(self):
        ...
        logging.debug('header is :{0}'.format(self.headers))
        logging.debug('uri params is:{0}'.format(show_json(self.params)))
    ...
    def get_curl_request(self):
        ...
        if self.cert:
            ...
            if self.verify:
                ...
                curl_str = curl_str + " -k"
        self.curl = curl_str
        logging.info("curl请求: {0}".format(self.curl))
    ...
    def request(self, method, url,
        ...
        logging.info("response响应: {0}".format(re))
        logging.debug('Response  is [{0}]:\n{1}'.format(self.response.status_code, re))
        self.clear_param()
