...
@author:colin.li
@time: 2021/08/05
@api: GET_/api/1/in/battery/get_national_status
@showdoc: http://showdoc.nevint.com/index.php?s=/252&page_id=31213
...
get_national_status_path = '/api/1/in/battery/get_national_status'
...
def create_battery_pack(env, mysql):
    ...
    with allure.step('在数据库中绑定车和电池，预换电车即只设置new_code'):
        ...
        data = {'vin': vin, 'vehicle_id': vid, 'code': result['gbt_code'], 'license_plate': None,
                ...
                'new_code': result['gbt_code'], 'pre_new_code': None, 'user_name': None}
...
def test_get_get_national_status(env, create_battery_pack):
    vin = env['vehicles']['colin']['vin']
    with allure.step(f'查询电车{vin}和包的绑定状态'):
        http = {
            "host": env['host']['tsp_in'],
            "path": get_national_status_path,
            "method": "GET",
            "headers": {"Content-Type": "application/json"},
            "params": {
                "app_id": app_id,
