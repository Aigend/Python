...
def test_history_info(env, mysql, case_name, target_app_id, host_key, data_key):
    ...
    group_dict = {"100417": tob_titan_group, "1000014": tob_titan_group, "1000003": toc_app_web_group,
                  "1000004": toc_app_web_group, "100404": toc_app_web_group}
    ...
    with allure.step(f'{host_key}公司员工获取消息内容接口{case_name}'):
        ...
        for data in datas:
            ...
            category_message_count_mysql = mysql[data_key].fetch(table,
                                                                 fields=['count(1) as message_count', "category"],
                                                                 where_model={'user_id': account_id,
                                                                              'app_id in': app_id_group,
                                                                              'category': category, "read": 0})
            assert int(category_message_count_mysql[0]["message_count"]) == int(unread_num)
            last_message_mysql = mysql[data_key].fetch(table,
                                                       fields=['message_id'],
                                                       where_model={'user_id': account_id, 'app_id in': app_id_group,
                                                                    'category': category},
                                                       suffix='order by  create_time desc limit 1')
