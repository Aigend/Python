...
@description:
    删除notify通知
    http://${host}/api/1/message/history_del
"""
import json
import time
import allure
...
class TestMeesageAPI(object):
    ...
    def test_history_del(self,env, mysql, prepare, prepare_message_id):
        """
        删除notify通知
        http://${host}/api/1/message/history_del

        客户端可以通过该接口删除某条消息，或者全部消息

        """
        inputs = {
            "host": env['host']['app_in'],
            "path": "/api/1/message/history_del",
            "method": "POST",
            "headers": {
        ...
        with allure.step("校验mysql,在数据表里只是标记visible=0，而不是真删除记录"):
            visible = mysql['nmp_app'].fetch(table, {'user_id': prepare['account_id'], 'message_id': prepare_message_id})[0]['visible']
