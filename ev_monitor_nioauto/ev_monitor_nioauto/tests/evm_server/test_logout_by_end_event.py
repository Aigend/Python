...
@author: chunming.liu
@Date: 2019/1/17 <br/>
@Feature: 1)存在于vehicle_profile表、存在于vehicle_platform_config表的私人领域车，成功转发到国家平台和地方平台车辆logout（不包含位置数据）
...
class TestVehicleLogout(object):
    ...
    @pytest.mark.parametrize("event_name", ['journey_end_event', 'charge_end_event'])
    def test_end_to_logout_nation(self, event_name, checker, prepare, publish_msg_by_kafka):
        platform_id = 156
        # 车辆信息
