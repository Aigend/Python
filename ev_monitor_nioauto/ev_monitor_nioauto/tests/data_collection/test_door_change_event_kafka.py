...
"""
:file: test_door_change_event_kafka.py
:author: chunming.liu
:Date: Created on 2019/1/12 下午7:12
:Description: 车门状态变更事件存储到kafka的swc-tsp-data_collection-test-vehicle_data供其他系统消费
"""
import allure
...
class TestDoorStatusChangeMsg(object):
    def test_door_status_change_event_kafka(self, vid, kafka, cmdopt, publish_msg_by_kafka):
        # 重置offset
        kafka['comn'].set_offset_to_end(kafka['topics']['vehicle_data'])

        # 构造并上报消息
        nextev_message, obj = publish_msg_by_kafka('door_change_event')
