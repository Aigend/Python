...
class TestUpdateTag(object):
    ...
    update_tag_cases = (
        ...
        ["反案例_其他100480app_id更新10000创建的tag", 100480, "user_id", "update_name", "http://www.updateurl.com", "new", {"result_code": "invalid_param", "debug_msg": "invalid tag id"}],
        ["反案例_type为None", 10000, "None", "update_name", "http://www.updateurl.com", "new",
         {"result_code": "invalid_param", "debug_msg": "necessary parameters are null or invalid."}],
        ["反案例_type为not_exist", 10000, "not_exist", "update_name", "http://www.updateurl.com", "new",
         {"result_code": "invalid_param", "debug_msg": "necessary parameters are null or invalid."}],
        ["反案例_name为None", 10000, "user_id", None, "http://www.updateurl.com", "new", {"result_code": "success"}],
        ["反案例_url为None", 10000, "user_id", "update_name", None, "new", {"result_code": "success"}],
        ["反案例_id为None", 10000, "user_id", "update_name", "http://www.updateurl.com", None, {"result_code": "invalid_param", "debug_msg": "invalid tag id"}],
    ...
    update_tag_negative_keys = "case_name,app_id,account_type,name,url,id_type,expect_result"
    update_tag_negative_cases = (
        ["反案例_type为None", 10000, "None", "update_name", "http://www.updateurl.com", "new", {"result_code": "invalid_param", "debug_msg": "necessary parameters are null or invalid."}],
        ["反案例_name为None", 10000, "user_id", None, "http://www.updateurl.com", "new", {"result_code": "success"}],
        ["反案例_url为None", 10000, "user_id", "update_name", None, "new", {"result_code": "success"}],
        ["反案例_id为None", 10000, "user_id", "update_name", "http://www.updateurl.com", None, {"result_code": "invalid_param", "debug_msg": "invalid tag id"}],
    )
    update_tag_negative_ids = [f"{case[0]}" for case in update_tag_negative_cases]

    @pytest.mark.parametrize(update_tag_negative_keys, update_tag_negative_cases, ids=update_tag_negative_ids)
    def test_update_tag_negative_case(self, env, mysql, case_name, app_id, account_type, name, url, id_type, expect_result, prepare_tag):
        with allure.step("准备tag id"):
            if id_type == "new":
                tag_id = prepare_tag
            elif id_type == "published":
        ...
        with allure.step(f'接口地址{tag_update_path}'):
            ...
            inputs["json"] = {k: v for k, v in inputs.get("json").items() if v}
            response = hreq.request(env, inputs)
            response.pop("request_id")
            response.pop("server_time")
            assert_equal(response, expect_result)
