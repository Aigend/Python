...
class TestSpecificEventMsg(object):
    @pytest.mark.skip
    def test_lv_batt_charging(self, publish_msg_by_kafka, checker):
        """
        For VMS
        BL260以后改用lv_batt_charging_event上报，sample_week和sample_month用于VMS筛选用，但目前已经弃用
        """
        # 上报
        nextev_message, obj = publish_msg_by_kafka('specific_event', event_type='lv_batt_charging', sleep_time=3)
    ...
    @pytest.mark.skip
    def test_nfc_op(self, publish_msg_by_kafka, checker):
        """
        ...
        """
        # 上报
        nextev_message, obj = publish_msg_by_kafka('specific_event', event_type='nfc_op')

        # # 校验
        tables = ['history_nfc_op']
        checker.check_mysql_tables(obj, tables)

    def test_fod_conf(self, vid, publish_msg_by_kafka, checker):
        # 上报
        nextev_message, fod_conf_in_message = publish_msg_by_kafka('specific_event', event_type='fod_conf')
    ...
    def test_max_charging_soc_event(self, redis_key_front, vid, publish_msg_by_kafka, checker):
        with allure.step("校验处理max soc specific event事件时，不再拦截历史时间戳"):
            """有变更就会上报，上报一次"""
            # 消息上报
            nextev_message, obj = publish_msg_by_kafka('specific_event', event_type='max_charging_soc_event')
            ...
            with allure.step("校验current_max_soc_value存入MySQL的状态表status_soc"):
                ...
                max_soc_in_mysql = checker.mysql.fetch('status_soc', {'id': vid}, ['max_soc'])[0]
            ...
            publish_msg_by_kafka('specific_event', event_type='max_charging_soc_event', data={'current_max_soc_value': 100})
    '''
    def test_swapping_in_redis(self, redis_key_front, vid, publish_msg_by_kafka, checker):
        with allure.step("校验当换电事件时，SpecialStatus会记录swapping 1/0=开始/结束，以及换电事件事件。连续相同的事件不会重复记录"):
            ...
            publish_msg_by_kafka('specific_event', event_type='power_swap_start')
            remote_vehicle_key_front = redis_key_front['remote_vehicle']
            ...
            publish_msg_by_kafka('specific_event', event_type=random.choice(['power_swap_end', 'power_swap_failure']))
            swapping_in_redis = json.loads(checker.redis.get(key))
            assert_equal(swapping_in_redis['swapping'], 0)
    '''
    def test_hv_battery_pre_heating(self, vid, publish_msg_by_kafka, checker):
        # 上报
        nextev_message, hv_battery_pre_heating_in_message = publish_msg_by_kafka('specific_event', event_type='hv_battery_pre_heating')
    ...
    def test_gd_alert_can_rate(self, vid, publish_msg_by_kafka, checker):
        # 上报
        nextev_message, gd_alert_can_rate_in_message = publish_msg_by_kafka('specific_event', event_type='gd_alert_can_rate')
    ...
    def test_deep_sleep(self, vid, publish_msg_by_kafka, checker):
        # 上报
        nextev_message, deep_sleep_in_message = publish_msg_by_kafka('specific_event', event_type='deep_sleep')
    ...
    def test_toby_event(self, vid, publish_msg_by_kafka, checker):
        # 上报
        event_type = "toby_event"
        nextev_message, toby_event_in_message = publish_msg_by_kafka('specific_event', event_type=event_type)
    ...
    def test_fota_trigger_event(self, vid, publish_msg_by_kafka, checker):
        # 上报
        nextev_message, fota_trigger_event_in_message = publish_msg_by_kafka('specific_event', event_type='fota_trigger_event')
    ...
    def test_fota_state_notify(self, vid, publish_msg_by_kafka, checker):
        # 上报
        nextev_message, fota_state_notify_in_message = publish_msg_by_kafka('specific_event', event_type='fota_state_notify')
    ...
    def test_bms_dtc_info(self, vid, publish_msg_by_kafka, checker):
        # 上报
        nextev_message, bms_dtc_info_in_message = publish_msg_by_kafka('specific_event', event_type='bms_dtc_info')
    ...
    @pytest.mark.skip
    def test_modem_event(self, vid, publish_msg_by_kafka, checker):
        # 上报
        nextev_message, modem_event_in_message = publish_msg_by_kafka('specific_event', event_type='modem_event')

        # 校验
        checker.check_mysql_tables(modem_event_in_message, ['history_modem_event'])

    @pytest.mark.skip
    def test_ble_op_event(self, vid, publish_msg_by_kafka, checker):
        # 上报
        nextev_message, ble_op_event_in_message = publish_msg_by_kafka('specific_event', event_type='ble_op_event')

        with allure.step('校验mysql history_ble_op'):
            checker.check_mysql_tables(ble_op_event_in_message, ['history_ble_op'])

    def test_car_key_settings_event(self, vid, publish_msg_by_kafka, checker):
        # 上报
        nextev_message, car_key_settings_event_in_message = publish_msg_by_kafka('specific_event', event_type='car_key_settings_event')
