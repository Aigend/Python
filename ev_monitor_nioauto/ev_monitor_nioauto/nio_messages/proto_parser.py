...
"""
:Description: convert protobuf to string, or convert dic to protobuf
"""
...
from nio_messages.pb2 import *
import base64

__all__ = ["parse_pb_string", "protobuf_to_dict", "TYPE_CALLABLE_MAP", "dict_to_protobuf", "REVERSE_TYPE_CALLABLE_MAP", "EXTENSION_CONTAINER"]
...
def update_init_file_with_protobuf_files(pb_path):
    """
    Args:
        pb_path: path containing the protobuf_files
    """
    pb_names = list()
    for item in os.listdir(pb_path):
        if item.endswith("_pb2.py"):
            pb_names.append(item[:-3])
    with open("%s/__init__.py" % pb_path, 'w') as ofile:
        ofile.write("#!/usr/bin/env python\n")
        ofile.write("#--*-- coding:utf-8 --*--\n\n\n")
        ofile.write("__all__ = %s" % pb_names.__str__())
...
def parse_pb_string(key_name, pb_str):
    ...
    try:
        ...
        # log("INFO", "Failed to parse the pb_str: %s" % pb_str)
        # log("INFO", "Exception: %s" % traceback.format_exc())
        return ret


def covert_dict_to_pb(pb_instance, dict_obj):
    """
    Args:
        pb_instance: pb instance you want corresponding to dict_obj
    """
    return dict_to_protobuf(pb_instance, dict_obj)
