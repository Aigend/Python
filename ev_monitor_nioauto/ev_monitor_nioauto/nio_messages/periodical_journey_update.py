...
上报时机：
    1.行驶过程中
    2.BMS（电池包监控）周期性（每隔几小时）唤醒整车检查电池包状态，持续10分钟,期间上报行程事件
上报频率：5s
"""
import time
import random
...
from nio_messages.pb2 import journey_update_msg_pb2
from nio_messages.data_unit import (gen_soc_status, gen_position_status, gen_vehicle_status,
                                    ...
                                    gen_body_status, gen_trip_status, gen_can_signal)
from nio_messages.proto_parser import protobuf_to_dict


def generate_message(vin, vid=None, protobuf_v=pb2.VERSION, reissue=False, icc_id=None, journey_id=None,
                     around_alarm=None, sample_points=None, config_version=None, signallib_version=None,
                     clear_fields=None):
    # JourneyUpdateMsg
    journey_update_msg = journey_update_msg_pb2.JourneyUpdateMsg()

    journey_update_msg.id = vin
    journey_update_msg.version = protobuf_v
    journey_update_msg.reissue = reissue
    journey_update_msg.icc_id = icc_id if icc_id else ('ICC' + vin)
    journey_update_msg.journey_id = journey_id if journey_id else (time.strftime("%Y%m%d", time.localtime()) + '00001')
    journey_update_msg.around_alarm = around_alarm if around_alarm else random.choice([0, 1])
    # journey_update_msg.config_version = config_version if config_version else 'ES8_0.6.1'
    journey_update_msg.signallib_version = signallib_version if signallib_version else 'BL00.06.01_DA_00'

    if sample_points is None:
        sample_points = [{}]

    for i in range(len(sample_points)):
        sample_point = journey_update_msg.sample_points.add()
        ...
        # trip_status
        sample_point.trip_status.MergeFrom(gen_trip_status(sample_points[i].get('trip_status', {})))

        # CanSignal
        sample_point.can_signal.MergeFrom(gen_can_signal(sample_points[i].get('can_signal', {})))

    if journey_update_msg.signallib_version < 'BL00.07.00_DA_01':
        eval('journey_update_msg.sample_points[0].ClearField("can_signal")')

    if clear_fields:
        for item in clear_fields:
            message, field = ('.'.join(item.split('.')[:-1]), item.split('.')[-1])
            message = message + '.' if message else ''
            eval('journey_update_msg.' + message + 'ClearField("' + field + '")')

    nextev_msg = gen_nextev_message('periodical_journey_update',
                                    journey_update_msg,
                                    journey_update_msg.sample_points[len(sample_points) - 1].sample_ts,
                                    version=protobuf_v,
                                    account_id=vid
                                    )

    journey_update_obj = pbjson.parse_pb_string("JourneyUpdateMsg", journey_update_msg.SerializeToString())

    return nextev_msg, journey_update_obj


def parse_message(message):
    event = journey_update_msg_pb2.JourneyUpdateMsg()
