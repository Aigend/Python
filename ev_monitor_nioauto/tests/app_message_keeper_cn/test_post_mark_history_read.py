...
def test_mark_history_read(env, cmdopt, mysql, case_name, target_app_id, host_key, data_key):
    with allure.step(f'{host_key}将消息标记未已读接口{case_name}'):
        ...
        table = f'history_{str(user_id)[-3:]}'
    ...
    with allure.step('校验数据库中标记为已读'):
        message_in_mysql = mysql[data_key].fetch(table, {'user_id': user_id, 'message_id': message_id, "read": 1}, retry_num=30)
        utc_time = now_utc_strtime()[:13]
        update_time = message_in_mysql[0].get("update_time")[:13]
    ...
    with allure.step('手动将数据标记改为未读'):
        if "stg" not in cmdopt:
            mysql[data_key].update(table, {"user_id": user_id, "message_id": message_id}, {"read": 0})
