...
class TestJourneyjourneyMsg(object):
    ...
    def test_periodical_journey(self, publish_msg, reissue, checker, vid, kafka, prepare):
        ...
        # charge_journey_obj['sample_points'][0]['driving_data'].pop('veh_dispd_spd')
        # charge_journey_obj['sample_points'][0]['driving_data'].pop('veh_outd_hum')
        ...
        tables = ['status_position', 'status_vehicle', 'status_soc',
                  ...
                  'status_tyre']
        for table in tables:
            assert len(checker.mysql.fetch(table, {'id': vid, 'sample_time': timestamp_to_utc_strtime(charge_journey_obj['sample_points'][0]['sample_ts'])},
                                           fields=['id'])) == 1
