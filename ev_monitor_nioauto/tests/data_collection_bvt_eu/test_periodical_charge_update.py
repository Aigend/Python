...
class TestChargeUpdateMsg(object):
    ...
    def test_periodical_charge_update(self, publish_msg, reissue, checker, vid, kafka, prepare):
        # 重置offset
        ...
        # charge_update_obj['sample_points'][0]['driving_data'].pop('veh_dispd_spd')
        # charge_update_obj['sample_points'][0]['driving_data'].pop('veh_outd_hum')
        ...
        for data in kafka['comn'].consume(kafka['topics']['vehicle_data'], timeout=10):
            ...
            if msg['publish_ts'] == charge_update_obj['sample_points'][0]['sample_ts'] and vid == msg['params']['account_id']:
                break
        ...
        tables = ['status_position', 'status_vehicle', 'status_soc',
                  ...
                  'status_tyre']
        for table in tables:
            assert len(checker.mysql.fetch(table, {'id': vid, 'sample_time': timestamp_to_utc_strtime(charge_update_obj['sample_points'][0]['sample_ts'])},
                                           fields=['id'])) == 1