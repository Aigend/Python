...
from data.email_content import html5_new, template_str_text, html5, html5_new1, markdown_file_img, markdown_file_form, \
    markdown_file_emoji, wechat_applet_template_dict
from tests.app_message_portal.template_management.template_server import create_template_and_published, \
    init_feishu_template
...
@pytest.mark.parametrize("case_name,channel,template_type,template_name,template_str", (
        ...
        ["正案例_feishu_text", "feishu", "text", f"【新增模板名称】_{random_string(13)}", template_str_text],
        ...
        ["反案例_channel字段为none", None, "text", f"【新增模板名称】_{random_string(13)}", template_str_text],
        ["反案例_type字段为none", "email", None, f"【新增模板名称】_{random_string(13)}", template_str_text],
        ["反案例_name字段为none", "email", "text", None, template_str_text],
        ["反案例_template_str字段为none", "email", "text", "name", None],
))
def test_add_template(env, cmdopt, mysql, case_name, channel, template_type, template_name, template_str):
    ...
    #     logger.debug(f"【{cmdopt}】环境暂不支持{channel}")
    #     return 0
    cmdopt = env["cmdopt"]
    ...
    with allure.step('添加消息模版接口'):
        ...
        if "正案例" in case_name:
            ...
            template = mysql["nmp_app"].fetch("message_template", {"name": template_name}, suffix="order by create_time desc")
...
template_str_1 = "这是一个测试电话请忽略。 [#city#][#deviceTypeName#]出现[#alarmName#]， 告警位于[#address#]。请立刻开展工作。"
template_str_2 = "这是一个测试电话请忽略。[#city#]出现告警，告警位于[#address#]。请立刻开展工作。"
template_str_3 = "这是一个测试电话请忽略。[#city#][#deviceTypeName#]出现告警，告警位于[#address#]。请立刻开展工作。"
template_str_4 = "这是一个压测内容请忽略。[#city#][#deviceTypeName#]出现告警，告警位于[#address#]。请立刻开展工作。"


@pytest.mark.skip("manual")
@pytest.mark.parametrize("case_name,channel,content_type,template_name,template_str",
                         (
                                 ["正案例_sms_text", "voice", "text", f"测试模板voice_alarm", template_str_1],
                                 ["正案例_sms_text", "voice", "text", f"APP模板voice_alarm", template_str_2],
                                 ["正案例_sms_text", "voice", "text", f"人工上报模板voice_alarm", template_str_3],
                                 # ["正案例_sms_text", "sms", "text", f"压测使用模板", template_str_4],
                         )
                         )
def test_template_add_publish(env, mysql, case_name, channel, content_type, template_name, template_str):
    tid = create_template_and_published(env, mysql, channel, content_type, template_name, template_str, app_id)
    logger.debug(f"{tid}:{template_str}")
