...
from tests.app_message_center.test_data.file.file_path import xlsx_path
...
class TestTemplateSendEmail(object):
    ...
    eu_email_with_attachment_send_keys = "case_name,template_name,replace_values,file_path"
    eu_email_with_attachment_send_cases = [
    ...
    eu_email_with_attachment_send_ids = [f"{case[0]}" for case in eu_email_with_attachment_send_cases]

    @pytest.mark.parametrize(eu_email_with_attachment_send_keys, eu_email_with_attachment_send_cases, ids=eu_email_with_attachment_send_ids)
    def test_template_with_attachment_email_send_eu(self, env, cmdopt, mysql, redis, case_name, template_name, replace_values, prepare_template, file_path):
        """
        ...
        with allure.step('CN邮件发送内容预览接口'):
            json_dict = {
                ...
                # "cc_recipients": cc_recipients,  暂不支持
                # "bcc_recipients": bcc_recipients, 暂不支持
            }
            inputs = {
                "host": env['host']['app_in'],
                "path": eu_email_with_attachment_send_path,
                "method": "POST",
                "params": {
                    "region": region,
                    "lang": "zh-cn",
