...
# @File : test_feishu_send_cn.py
# @Author : qiangwei.zhang
# @time: 2021/08/25
# @api: POST_/api/2/in/message_portal/template/cn/feishu/send 【必填】
...
eu_fei_shu_send_path = "/api/2/in/message_portal/template/eu/feishu/send"
cn_fei_shu_send_path = "/api/2/in/message_portal/template/cn/feishu/send"
test_data_map = {"recipients": "qiangwei.zhang@nio.com", "user_ids": "1234,2345", "account_ids": "1234,2345"}
app_id = 10000
...
class TestTemplateSend(object):
    ...
    fei_shu_send_cn_keys = "case_name,template_name,replace_values,user_key"
    fei_shu_send_cn_cases = [
    ...
    fei_shu_send_cn_ids = [f"{case[0]}" for case in fei_shu_send_cn_cases]

    @pytest.mark.parametrize(fei_shu_send_cn_keys, fei_shu_send_cn_cases, ids=fei_shu_send_cn_ids)
    def test_fei_shu_send_cn(self, env, cmdopt, mysql, case_name, template_name, replace_values, prepare_template, user_key):
        """
        ...
        with allure.step('CN短信发送内容预览接口'):
            ...
            assert len(response.get("data")) == len(user_info.split(","))

    fei_shu_send_eu_keys = "case_name,template_name,replace_values,user_key"
    fei_shu_send_eu_cases = [
        # ("正案例_无变量模板", "FEISHU_no_variable", None, "recipients"),
        # ("正案例_只有公共变量模板_变量无重复", "FEISHU_R_no_repeat", None, "recipients"),
        # ("正案例_只有公共变量模板_变量有重复", "FEISHU_R_repeat", None, "recipients"),
        # ("正案例_只有自定义变量模板_变量无重复", "FEISHU_L_no_repeat", "all", "recipients"),
    ...
    fei_shu_send_eu_ids = [f"{case[0]}" for case in fei_shu_send_eu_cases]

    @pytest.mark.parametrize(fei_shu_send_eu_keys, fei_shu_send_eu_cases, ids=fei_shu_send_eu_ids)
    def test_fei_shu_send_eu(self, env, cmdopt, mysql, case_name, template_name, replace_values, prepare_template, user_key):
        """
            接口文档 http://showdoc.nevint.com/index.php?s=/647&page_id=32599
        """
