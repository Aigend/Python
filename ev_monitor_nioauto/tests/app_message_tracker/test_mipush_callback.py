...
mi_push_callback_path = "/api/2/message_tracker/m_cb"
...
def test_mi_push_callback(env, mysql, cmdopt, category, channel, host_key, data_key, ):
    num = 10
    brand = 'xiaomi'
    ...
    time.sleep(10)
    data = {
        "msgId1": {"param": f"{message_id}", "type": 1, "targets": ','.join([user.get("device_token") for user in user_infos[0:5]])},
        "msgId2": {"param": f"{message_id}", "type": 2, "targets": ','.join([user.get("device_token") for user in user_infos[5:]])}
    }
    app_id = 10000
    inputs = {
        "host": env['host']['app_ex'],
        "path": mi_push_callback_path,
        "method": "POST",
        "headers": {"Content-Type": "application/x-www-form-urlencoded"},
