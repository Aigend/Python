...
def perform(msg=Message.Message()):
    log.info("<Cgw> <prepare> enter cgw prepare perform function")
    ...
    log.info(f"<Cgw> <prepare> request_url:{request_url}")
    request_headers = {
    ...
    log.info(f"<Cgw> <prepare> response code:{response.status}, response reason:{response.reason}")
    data = response.read()
    log.info(f"<Cgw> <prepare> response data:{str(data)}")
