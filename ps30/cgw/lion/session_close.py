...
def perform(msg=Message.Message()):
    log.info("<Cgw> <session_close> enter cgw session_close perform function")
    ...
    log.debug(f"<Cgw> <session_close> request_url:{request_url}")
    request_headers = {
    ...
    body = {
        ...
        "event_message": "session_close success"
    }
    ...
    log.debug(f"<Cgw> <session_close> response code:{response.status}, response reason:{response.reason}")
    data = response.read()
    log.debug(f"<Cgw> <session_close> response data:{str(data)}")
