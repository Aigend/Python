...
def perform(msg=Message.Message()):
    log.info("<Cgw> <sleep> enter cgw sleep perform function")
    ...
    log.debug(f"<Cgw> <sleep> request_url:{request_url}")
    request_headers = {
    ...
    log.debug(f"<Cgw> <sleep> response code:{response.status}, response reason:{response.reason}")
    data = response.read()
    log.debug(f"<Cgw> <sleep> response data:{str(data)}")
