...
def perform(msg=Message.Message()):
    log.info("<Cgw> <psap_status> enter cgw psap_status perform function")
    session_id = msg.get_session_id()

    # Defining parts of the HTTP request
    request_url = basic_info.get_station_authority() + '/v1/api/psap_status' + '?session_id=' + session_id
    log.debug(f"<Cgw> <psap_status> request_url:{request_url}")
    # request_headers = {
    ...
    log.debug(f"<Cgw> <psap_status> response code:{response.status}, response reason:{response.reason}")
    data = response.read()
    log.debug(f"<Cgw> <psap_status> response data:{str(data)}")
