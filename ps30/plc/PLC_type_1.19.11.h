...
typedef enum
{
	...
	PLC_AXIS_ID_60 								= 60,// 挂电池时自动加锁，虚拟轴
...
typedef enum
{
	...
	PLC_MOTION_ID_HANG_BMS_AUTO_LOCK			= 268,// 挂电池时自动加锁
	PLC_MOTION_ID_MAX
} PLC_MOTION_ID_ENUM;
...
typedef enum
{
	...
	PLC_CMD_ID_SWAP_ACTION_A                 	= 34, // 车站并行动作命令A + param1:1-成功,2-失败;(注:P1->开合门打开、RGV上升)
  	PLC_CMD_ID_SWAP_ACTION_B                 	= 35, // 车站并行动作命令B + param1:1-成功,2-失败;(注:P2->解锁)
  	PLC_CMD_ID_SWAP_ACTION_C                 	= 36, // 车站并行动作命令C + param1:1-成功,2-失败;(注:P3->RGV下降)
	PLC_CMD_ID_MAX
}PLC_CMD_ID_ENUM;
...
{	
    ...
    int gun_1_sign_torque;                    		// 1枪标定扭矩
    int gun_2_sign_torque;                    		// 2枪标定扭矩
    int gun_3_sign_torque;                    		// 3枪标定扭矩
    int gun_4_sign_torque;                    		// 4枪标定扭矩
    int gun_5_sign_torque;                    		// 5枪标定扭矩
    int gun_6_sign_torque;                    		// 6枪标定扭矩
    int gun_7_sign_torque;                    		// 7枪标定扭矩
    int gun_8_sign_torque;                    		// 8枪标定扭矩
    int gun_9_sign_torque;                    		// 9枪标定扭矩
    int gun_10_sign_torque;                   		// 10枪标定扭矩
    int gun_11_sign_torque;                   		// 1‘枪标定扭矩
    int gun_12_sign_torque;                   		// 2’枪标定扭矩                             
	int reserved[5];                         		// 预留                                          
}PLC_SETTING_STRUCT;
...
/**
 *@brief PLC车站并行动作命令状态反馈
 */
typedef struct 
{
	int8_t cmd_a;													// 车站并行动作命令A状态反馈，1:成功，2:失败
	int8_t cmd_b;													// 车站并行动作命令B状态反馈，1:成功，2:失败
	int8_t cmd_c;													// 车站并行动作命令C状态反馈，1:成功，2:失败
}PLC_FB_SWAP_CMD_RESULT_STRUCT;
...
typedef struct 
{
	int8_t rgv_position;													// 	0:非特殊位置 1:加解锁平台在工作位-10mm及以上 2: 加解锁平台在卡销位+10mm及以下							// 车站并行动作命令C状态反馈，1:成功，2:失败
}PLC_FB_RGV_POSITION_STRUCT;
...
typedef struct 
{
	...
	PLC_FB_SWAP_CMD_RESULT_STRUCT	fb_swap_cmd;					// PLC车站并行动作命令状态反馈
	PLC_FB_RGV_POSITION_STRUCT		fb_rgv_position;				// PLC的RGV位置反馈
	int8_t reserved[21];											// 预留
